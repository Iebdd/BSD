...
\begin{document}
	...
	\subsection{Minimales Kostenflussproblem}
	Bei dem minimalen Kostenflussproblem werden zusätzlich zu dem Fluss wie bei dem maximalen Flussproblem die Kosten angegeben. Eine weitere Eigenschaft ist der negative Kreis, was ein Kreis in einem Graphen ist, für den die Kosten negativ sind. (Also ein Pfad mit gleichem Start- und Endpunkt wobei die Kosten negativ sind). Der Ablauf ist folgender:
	\begin{enumerate}
		\item{Ein Fluss sollte gegeben sein (Dieser kann ein Fluss oder ein maximaler Fluss sein)}
		\item{Man muss den Inkrementgraphen bilden}
		\item{Solange der Graph negative Kreise enthält:}
		\begin{itemize}
			\item{}
		\end{itemize}
	\end{enumerate}
	\section{Matchingproblem}
	Bei dem Matchingproblem verbindet man die Knoten eines Bipartiten Graphen. Dabei werden alle Knoten nur mit Knoten auf der anderen Seite verbunden. Am Ende sollen möglichst viele Knoten verbunden werden, wobei ein Knoten jeweils nur eine Verbindung haben darf. Ein Beispiel dafür ist die Jobsuche. Dabei modelliert man Jobsuchende mit Jobs, wobei die Kanten die Bewerbungen sind.
